...
public partial class complaintlistforworkorder : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
{
    ...
    try
    {
        if (Session["UserID"].ToString() == null || Session["UserID"].ToString() == "")
        {
            ...
            chkLoadInspection.Checked = false;
            if (!IsPostBack)
            {
                FetchComplaintOutstandingDetails();
                FetchInspectionOutstandingDetails();
            }
        }
    catch (Exception ex)
    {
...
private void FetchComplaintOutstandingDetails()
{
    try
    {
        ...
        //cmSQL.CommandText = "SELECT * FROM dComplaint where not(OwnedBy='' or OwnedBy is null) and RefNo NOT IN (SELECT ComplaintRef FROM dWorkorder) order by TransDate ASC";
         cmSQL.CommandText = "SELECT * FROM dComplaint where OwnedByAccepted=1 and RefNo NOT IN (SELECT ComplaintRef FROM dWorkorder Where Source='Complaint') order by TransDate ASC";
...
protected void btnNewworkorder_Click(object sender, EventArgs e)
{
        if (dsource.Value == "Complaint")
        {
            ...
            Session["dsource"] = "Complaint";
        }
        {
            Session["ComplaintRefno"] = Inspection_refno.Value;
            Session["ClientInfo"] = client_name1.Value;
            Session["ProductInfo"] = property_name1.Value;
            Session["ComplaintScope"] = Scope_name1.Value;
            Session["ContractRefNo"] = contract_ref1.Value;
            Session["dsource"] = "Inspection";
        }
        Response.Redirect("workorder.aspx", true);
}

protected void btnBasicWorkorder_Click(object sender, EventArgs e)
{
    ...
    private void FetchInspectionOutstandingDetails()
    {
        try
        {
            string dCnStr = Session["Cnn"].ToString();
            ...
            SqlCommand cmSQL = cnSQL.CreateCommand();
            SqlDataReader drSQL = null;
            cmSQL.CommandText = "SELECT * FROM dInspection where RefNo NOT IN (SELECT ComplaintRef FROM dWorkorder Where Source='Inspection') order by TransDate ASC";
    ...
    s++;
                ...
                createHTMLelements4Inspection(drSQL["Refno"].ToString(), drSQL["InspectionDetails"].ToString(), dp, durate, colorstatus, drSQL["InspectionScope"].ToString(), drSQL["ClientRefNo"].ToString() + " - " + drSQL["ClientName"].ToString(), drSQL["Username"].ToString(), date1, drSQL["ContractRef"].ToString(), drSQL["InitialActionTaken"].ToString(), drSQL["InspectionTeam"].ToString(), drSQL["ClientUpdate"].ToString());
    ...
    private void createHTMLelements4Inspection(string RefNo, string dComment, string dProduct, string dDuration, int colorScheme, string dscope, string client, string complainBy, DateTime complainDate, string dcontractRef,string dactiontaken,string dteam, string dclientupdate)
    {
        ...
        myAnchor.Attributes.Add("data-whatever", RefNo);
        //myAnchor.Attributes.Add("onclick", "opendetailscreen()");
        divInspection.Controls.Add(myAnchor);
        ...
        HtmlInputText myInput1 = new HtmlInputText();
        myInput1.ID = RefNo + "a1";
        myInput1.Value = dscope;
        dynDiv2.Controls.Add(myInput1);

        HtmlInputText myInput2 = new HtmlInputText();
        myInput2.ID = RefNo + "b1";
        myInput2.Value = client;
        dynDiv2.Controls.Add(myInput2);

        HtmlInputText myInput3 = new HtmlInputText();
        myInput3.ID = RefNo + "c1";
        myInput3.Value = complainBy;
        dynDiv2.Controls.Add(myInput3);

        HtmlInputText myInput4 = new HtmlInputText();
        myInput4.ID = RefNo + "d1";
        myInput4.Value = complainDate.ToString();
        dynDiv2.Controls.Add(myInput4);


        HtmlInputText myInput5 = new HtmlInputText();
        myInput5.ID = RefNo + "e1";
        myInput5.Value = Session["ImagePath4Display"] + RefNo + ".png";
        dynDiv2.Controls.Add(myInput5);

        HtmlInputText myInput6 = new HtmlInputText();
        myInput6.ID = RefNo + "f1";
        myInput6.Value = dComment;
        dynDiv2.Controls.Add(myInput6);

        HtmlInputText myInput7 = new HtmlInputText();
        myInput7.ID = RefNo + "g1";
        myInput7.Value = dProduct;
        dynDiv2.Controls.Add(myInput7);

        HtmlInputText myInput8 = new HtmlInputText();
        myInput8.ID = RefNo + "h1";
        myInput8.Value = dcontractRef;
        dynDiv2.Controls.Add(myInput8);

        HtmlInputText myInput9 = new HtmlInputText();
        myInput9.ID = RefNo + "i1";
        myInput9.Value = dactiontaken;
        dynDiv2.Controls.Add(myInput9);

        HtmlInputText myInput10 = new HtmlInputText();
        myInput10.ID = RefNo + "j1";
        myInput10.Value = dteam;
        dynDiv2.Controls.Add(myInput10);

        HtmlInputText myInput11 = new HtmlInputText();
        myInput11.ID = RefNo + "k1";
        myInput11.Value = dclientupdate;
